...
- [x] 字符串相关操作
  ...
  - FormatSQL 格式化SQL，输入一个SQL，然后得到已经格式化好的、易读的SQL语句。
- [x] 本地缓存相关操作
...
- [x] Slice切片相关
  ...
  - MergeSlice 多个切片合并，免去平时我们要多次循环append的麻烦，直接将多个切片一次性传入这个函数，最终得到一个合并好的最终切片，同时解决了内存扩容的性能问题。
  - ReplaceOrAppend 替换或追加一个元素到切片，存在时替换，不存在时追加，这个函数和ReplaceOrAppendFunc相比，调用非常简单，但不适合非常大的切片。
  - ReplaceOrAppendFunc 替换或追加一个元素到切片，存在时替换，不存在时追加，这个函数和ReplaceOrAppend相比，调用非常略微复杂，要传一个回调函数，但适合非常大的切片，性能高。
- [x] 结构体相关操作
...
- [x] UUID V7 更好用的高性能UUID
    - UuidV7 通用方法，自带缓冲池，不需要初始化，随时获得ID，并发安全。
    - SimpleUuidV7 去除横线方法，自带缓冲池，不需要初始化，到处随时获得ID，并发安全，推荐👍👍👍。
- [x] 时间函数
